...
The two public inquiry URLs use the same validation, spam controls, and database transaction. Success means the inquiry and two notification jobs have committed to Neon. Email is attempted after that response, with a five-minute Vercel cron recovering interrupted or failed attempts.
...
- Cron: `/api/cron/leads`, every five minutes, requires the platform-supplied `CRON_SECRET` authorization header. It retries, reconciles delivery events, removes expired records, and saves a timestamped status summary.
...
Default status output contains no contact details. Supplying a reference intentionally reveals that inquiry's details locally. The last scheduled check should be within ten minutes; a stale timestamp needs investigation in Vercel Cron/Runtime Logs. Look for `[leads] intake_unavailable`, `scheduled_check_failed`, or `delivery_attention`.
